...
static void init_audio(Game *game)
{
  ...
  game->audio.id = SDL_OpenAudioDevice(NULL, 0,
                                       &game->audio.device_spec,
...
static int play_explosion_sound(void *g)
{
  Game *game = (Game *)g;

  game->in_thread = SDL_TRUE;
  SDL_ClearQueuedAudio(game->audio.id);
  SDL_QueueAudio(game->audio.id,
                 game->audio.audio_info[0].wave_buffer,
                 game->audio.audio_info[0].wave_length);
...
static int load_audio(Game *game, const char *path)
{
  ...
  audio_spec = SDL_LoadWAV(path,
                    &game->audio.audio_info[game->audio.index].audio_spec,
                    &game->audio.audio_info[game->audio.index].wave_buffer,
                    &game->audio.audio_info[game->audio.index].wave_length);

  if (audio_spec == NULL) {
    return 1;
  }

  SDL_BuildAudioCVT(&cvt, audio_spec->format, audio_spec->channels,
                    audio_spec->freq, game->audio.device_spec.format,
                    game->audio.device_spec.channels,
                    game->audio.device_spec.freq);

  if (cvt.needed) {
    cvt.len = game->audio.audio_info[game->audio.index].wave_length;
    cvt.buf = (Uint8 *)SDL_malloc(cvt.len * cvt.len_mult);
    memcpy(cvt.buf, game->audio.audio_info[game->audio.index].wave_buffer,
           game->audio.audio_info[game->audio.index].wave_length);
...
static int check_dimensions(Game *game)
{
  ...
  status = SDL_GetDisplayBounds(0, &rect);
...
{
  return !(s2->x > (s1->x + s1->width) || (s2->x + s2->width) < s1->x ||
           s2->y > (s1->y + s1->height) || (s2->y + s2->height) < s1->y);
}

// e.g. load_image_with_index("/usr/share/game/alien.png", 2);
// This would get /usr/share/game/alien02.png if it exists.
static SDL_Surface *load_image_with_index(Game *game, char *path,
                                          unsigned int indx)
{
  SDL_Surface *surface;
  size_t path_len;
  ...
  surface = NULL;
  path_len = strlen(path) + 3;
  ext = strrchr(path, '.');
  filename = (char *)malloc(path_len * sizeof(char));
  ...
  snprintf(filename, path_len, "%.*s%02d%s",
           (int)(ext - path), path, indx, ext);
...
static int load_sprite(Game *game, Sprite *sprite, char *path)
{
  ...
  while ((surface = load_image_with_index(game, path, indx)) != NULL) {
    sprite->texture = realloc(sprite->texture,
                              sizeof(SDL_Texture **) * (indx + 1));
    ...
    sprite->texture[indx] = SDL_CreateTextureFromSurface(game->renderer,
                                                         surface);
...
static void draw_sprite(Game *game, Sprite *sprite)
{
  if (sprite->is_visible) {
    SDL_Rect drect = { (int)sprite->x, (int)sprite->y,
                       sprite->width, sprite->height };

    SDL_RenderCopy(game->renderer, sprite->texture[sprite->current_frame],
                   NULL, &drect);
...
static int init_bigblue(Game *game)
{
  ...
  status = initialise_sprite(game,
                             &game->bigblue.sprite, DATADIR"/bigblue.png");
...
static int init_player(Game *game)
{
  ...
  status = initialise_sprite(game, &game->player.sprite, DATADIR"/player.png");
...
static int init_alien_type(Game *game, int indx, char *path)
{
  ...
  status = initialise_sprite(game, &game->alien_sprite[indx], path);

  if (status != 0) {
    return status;
  }

  for (i = 0; i < game->alien_count; i++) {
...
{
  int i, j, leader_x, leader_y;

  leader_x = 5;
  leader_y = 20;

  for (i = 0; i < game->alien_type; i++) {
    for (j = 0; j < game->alien_count; j++) {
      ...
      game->alien[i][j].sprite.x = leader_x + j *
                                   (game->alien[i][j].sprite.width + 20.0);
      game->alien[i][j].sprite.y = leader_y + (i + 1) *
                                   (game->alien[i][j].sprite.height + 20.0);
    }
  }
}
...
static int init_aliens(Game *game)
{
  ...
  count = 0;
  status = init_alien_type(game, count++, DATADIR"/purple.png");
...
static int init_explosion(Game *game)
{
  ...
  status = initialise_sprite(game, &game->explosion, DATADIR"/explosion.png");
...
static int init_missile(Game *game)
{
  ...
  status = initialise_sprite(game, &game->missile, DATADIR"/missile.png");
...
static int init_playmis(Game *game)
{
  ...
  status = initialise_sprite(game, &game->playmis, DATADIR"/playmis.png");
  game->playmis.x = game->playmis.y = 0;
  game->playmis.is_visible = SDL_FALSE;
...
static int init_line(Game *game)
{
  ...
  status = initialise_sprite(game, &game->line, DATADIR"/line.png");
  game->line.x = 50;
  game->line.y = LINE_Y;
...
static void reset_asteroid_quarters(Game *game)
{
  ...
  x = game->asteroid.sprite.x;
  y = game->asteroid.sprite.y;
  init_craft(&game->ul);
  init_craft(&game->ur);
  init_craft(&game->ll);
  init_craft(&game->lr);
...
static int init_asteroid_quarters(Game *game)
{
  ...
  status = initialise_sprite(game, &game->ul.sprite, DATADIR"/ul.png");
...
static void explode(Game *game, Craft *craft)
{
  ...
  game->explosion.x = craft->sprite.x + craft->sprite.width / 2 -
                      game->explosion.width / 2;
  game->explosion.y = craft->sprite.y + craft->sprite.height / 2 -
                      game->explosion.height / 2;
...
static void create_pause_screen(Game *game)
{
  if (game->paused) {
    ...
    format = SDL_GetWindowPixelFormat(game->window);

    if (format != SDL_PIXELFORMAT_UNKNOWN) {
      capture = SDL_CreateRGBSurfaceWithFormat(0, game->width, game->height,
                                               SDL_BITSPERPIXEL(format), format);
    } else {
      fprintf(stderr, "%s: %s\n", game->title, SDL_GetError());
      format = SDL_PIXELFORMAT_ARGB8888;
      capture = SDL_CreateRGBSurface(0, game->width, game->height, 32,
    ...
    SDL_RenderReadPixels(game->renderer, NULL, format,
...
static int handle_key_down(Game *game, SDL_Event *event)
{
  ...
  if (game->paused && event->key.keysym.scancode != SDL_SCANCODE_P &&
...
static SDL_Texture *create_text_texture(Game *game, char *text)
{
  ...
  surface = TTF_RenderText_Solid(game->font, text, colour);
...
static void draw_lives(Game *game)
{
  ...
  sx = game->player.sprite.x;
  sy = game->player.sprite.y;
  game->player.sprite.x = game->width / 2 -
                          ((game->player.sprite.width + 2) * game->lives) / 2;
  game->player.sprite.y = 10;
  inc = game->player.sprite.width + 2;
...
static void draw_score_digits(Game *game)
{
  ...
  for (i = 0; i < 7; i++) {
    set_rect(drect, 5 + span, 1, game->score.width[game->score.score_digit[i]],
             game->score.height[game->score.score_digit[i]]);
    SDL_RenderCopy(game->renderer,
                   game->score.digit[game->score.score_digit[i]], NULL, &drect);
...
static void draw_high_score_digits(Game *game)
{
  ...
  for (i = 0; i < 7; i++) {
    set_rect(drect, WIDTH - 120 + span, 1,
             game->score.width[game->score.high_digit[i]],
             game->score.height[game->score.high_digit[i]]);
    SDL_RenderCopy(game->renderer,
                   game->score.digit[game->score.high_digit[i]], NULL, &drect);
...
static void move_big_blue_missiles(Game *game)
{
  ...
  // Fire bigblue missiles at random if bigblue visible and not already fired.
  if ((rand() & 1023) < game->level && !game->big_blue_missiles.is_visible &&
      game->bigblue.sprite.is_visible) {
  ...
  // Test if missiles have hit player.
...
static void check_if_player_missile_hit_bigblue(Game *game)
{
  if (game->bigblue.sprite.is_visible &&
      has_intersection(&game->bigblue.sprite, &game->playmis)) {
    game->playmis.is_visible = SDL_FALSE;
...
static void move_asteroid(Game *game)
{
  if (game->asteroid.sprite.is_visible) {
    ...
    if (game->asteroid.sprite.x > game->width ||
        game->asteroid.sprite.y > game->height ||
        game->asteroid.sprite.x < -game->asteroid.sprite.width) {
      game->asteroid.sprite.is_visible = SDL_FALSE;
    }
  }
}
...
static void show_game_over_message(Game *game)
{
  ...
  set_rect(rect, game->width / 2 - width / 2,
           game->height / 2 - height / 2 - 40, width, height);
...
static void show_paused_message(Game *game)
{
  ...
  if (width[0] == 0) {
    for (i = 0; i < PAUSE_MSG; i++) {
      get_texture_dimensions(game->paused_message[i], &width[i], &height[i]);
    }
  }

  for (i = 0; i < PAUSE_MSG; i++) {
    set_rect(rect, game->width / 2 - width[i] / 2,
             game->height / 2 - height[i] / 2 + hp, width[i], height[i]);
  ...
  is_visible = game->playmis.is_visible;
  game->playmis.is_visible = SDL_TRUE;
  draw_sprite(game, &game->playmis);
...
static int play_game(Game *game)
{
  ...
  SDL_Event event;
  Uint32 frame_delay;
  Uint64 diff, start_time;
  struct timespec ts;

  ts.tv_sec = 0;
  frame_delay = (Uint32)((double)SDL_GetPerformanceFrequency() / (double)FPS);
  one_ns = 0.000000001 * (double)SDL_GetPerformanceFrequency();
  ...
  while (1) {
    ...
    if (diff < frame_delay) {
      ts.tv_nsec = (frame_delay - diff) * one_ns;
      nanosleep(&ts, NULL); // SDL_Delay results in jerky graphics.
    }
...
static int init_textures(Game *game)
{
  ...
  status = create_text_textures(game);

  if (status != 0) {
    return 1;
  }

  /* Get widths and heights of digit textures. */
  for (i = 0; i < 10; i++) {
    SDL_QueryTexture(game->score.digit[i], &format,
                     &acc, &game->score.width[i], &game->score.height[i]);
  }

  game->game_over_message =
    create_text_texture(game, "Game Over! (Press n for new game)");
...
static void free_graphics(Game *game)
{
  ...
  for (i = 0; i < ALIEN_TYPE; i++) {
    int j;

    for (j = 0; j < ALIEN_POPULATION; j++) {
